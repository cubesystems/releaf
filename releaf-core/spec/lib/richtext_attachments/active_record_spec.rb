...
describe Releaf::RichtextAttachments::ActiveRecord do
  ...
  it "creates attachments relation" do
    expect( Book.reflect_on_association :releaf_richtext_attachments ).to_not be nil
  end
  ...
  describe "#releaf_richtext_attachment_uuids" do
    it "returns all richtext field content data-attachment-id attribute values" do
      ...
      expect( book.send(:releaf_richtext_attachment_uuids).size ).to eq(2)
      expect( book.send(:releaf_richtext_attachment_uuids) ).to include(uuid_1, uuid_2)
    end

    it "calls #richtext_columns" do
      book = Book.new
      expect(book).to receive(:richtext_columns).once.and_return([])
      book.send(:releaf_richtext_attachment_uuids)
    end
  end

  describe "#maintain_releaf_richtext_attachments" do
    ...
    let(:attachment_1) do
      Releaf::RichtextAttachment.create(file: file)
    end
    let(:attachment_2) do
      Releaf::RichtextAttachment.create(file: image)
    end

    it "is called after save" do
      expect(book).to receive(:maintain_releaf_richtext_attachments).once
      book.save!
    end

    it "calls #releaf_richtext_attachment_uuids" do
      expect(book).to receive(:releaf_richtext_attachment_uuids).once
      book.send(:maintain_releaf_richtext_attachments)
    end
    ...
    context "when new attachments are detected" do
      it "assigns new attachments to book" do
        ...
        expect( attachment_1.owner_type ).to be_nil
        expect( attachment_1.owner_id ).to be_nil
        expect( attachment_2.owner_type ).to be_nil
        expect( attachment_2.owner_id ).to be_nil

        expect { book.save! }.to change { book.releaf_richtext_attachments.count }.from(0).to(2)
        attachment_1.reload
        attachment_2.reload

        expect( attachment_1.owner_type ).to eq 'Book'
        expect( attachment_1.owner_id ).to eq book.id
        expect( attachment_2.owner_type ).to eq 'Book'
        expect( attachment_2.owner_id ).to eq book.id
      end
    end
    ...
    context "when some attachments are removed" do
      ...
      it "deletes removed attachments" do
        ...
        expect { book.save! }.to change { book.releaf_richtext_attachments.count }.from(2).to(1)

        expect( attachment_1.owner_type ).to eq 'Book'
        expect( attachment_1.owner_id ).to eq book.id

        expect( Releaf::RichtextAttachment.where(uuid: attachment_2.uuid).exists? ).to be false
      end
    end
    ...
    context "when attachments didn't change" do
      ...
      it "does nothing" do
        ...
        expect { book.save! }.to_not change { book.releaf_richtext_attachments.count }
