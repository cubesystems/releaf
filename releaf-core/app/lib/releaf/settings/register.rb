class Releaf::Settings::Register
  ...
  def normalize(item)
    ...
    raise Releaf::Core::Error, "Unsupported settings type: #{item[:type]}" unless settings_class.supported_types.include?(item[:type])
    raise Releaf::Core::Error, "Dissallowed settings keys: #{dissallowed_keys}" if dissallowed_keys.present?
